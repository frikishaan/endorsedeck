<x-guest-layout>
    <div class="pt-4 bg-gray-100">
        <div class="min-h-screen flex flex-col items-center pt-3 sm:pt-0">
            <div>
                @if ($wall->logo)
                    <img src="{{ $wall->logo }}" alt="{{$wall->name}} logo" width="100px" height="100px" class="rounded-md">
                @endif
            </div>
                    ...
                    <div class="w-full mx-auto rounded-lg bg-white shadow p-5 text-gray-800 break-inside mb-6" style="max-width: 400px">
                        <div class="w-full flex mb-4">
                            <div class="overflow-hidden rounded-full w-12 h-12">
                                <img src="{{$testimonial->user['avatar'] == "" ? 'https://ui-avatars.com/api/?name='. $testimonial->user['name'] .'&color=7F9CF5&background=EBF4FF' : $testimonial->user['avatar'] }}" alt="{{$testimonial->user['name']}}">
                            </div>
